...
interface CartItemProps {
  item: CartItemType;
  onUpdate: (productId: string, quantity: number, variant?: string) => void;
...
export function CartItem({ item, onUpdate, onRemove }: CartItemProps) {
  return (
    <div className="flex flex-col gap-4 rounded-[2rem] border border-slate-200 bg-white p-6 sm:flex-row sm:items-center">
      ...
      <button type="button" onClick={() => onRemove(item.productId, item.variant)} className="rounded-full border border-red-200 bg-red-50 p-2 text-red-600 transition hover:bg-red-100">
        <Trash2 size={16} />
      </button>
    </div>
  );
