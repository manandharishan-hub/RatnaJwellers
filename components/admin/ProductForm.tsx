    ...
    <form onSubmit={onSubmit} className="space-y-8 rounded-[2rem] border border-slate-200 bg-white p-8 shadow-sm">
      ...
      <div className="grid gap-5 md:grid-cols-3">
        <div>
          ...
          <Input id="price" type="number" min="1" step="1" value={toNprInputValue(form.price)} onChange={(event) => updateField("price", fromNprInputValue(event.target.value))} placeholder="25000" required />
        </div>
        <div>
          <Label htmlFor="comparePrice">Old price (NPR)</Label>
          <Input id="comparePrice" type="number" min="0" step="0.01" value={toNprInputValue(form.comparePrice)} onChange={(event) => updateField("comparePrice", fromNprInputValue(event.target.value))} placeholder="30000" />
        </div>
        <div>
          <Label htmlFor="costPrice">Cost price (NPR)</Label>
          <Input id="costPrice" type="number" min="0" step="0.01" value={toNprInputValue(form.costPrice)} onChange={(event) => updateField("costPrice", fromNprInputValue(event.target.value))} placeholder="18000" />
        </div>
      </div>
      ...
      <div className="grid gap-5 md:grid-cols-4">
        ...
        <div>
          <Label htmlFor="gemstone">Stone</Label>
      ...
      <div className="space-y-4 rounded-3xl border border-slate-200 bg-slate-50 p-5">
        <div className="flex items-center justify-between">
          <h2 className="font-semibold text-[#0A1628]">Product photos</h2>
        ...
        <div className="flex gap-3">
          <Input value={imageUrl} onChange={(event) => setImageUrl(event.target.value)} placeholder="Paste photo link" />
          <button type="button" onClick={() => addImage(imageUrl)} className="rounded-full border border-slate-300 px-4 py-2 text-sm font-semibold text-[#0A1628]">Add photo</button>
        </div>
      ...
      <div className="space-y-4 rounded-3xl border border-slate-200 bg-slate-50 p-5">
        <div className="flex items-center justify-between">
          <h2 className="font-semibold text-[#0A1628]">Sizes and stock</h2>
        ...
        {form.variants.map((variant, index) => (
          <div key={index} className="grid gap-3 md:grid-cols-[1fr_160px_auto]">
            ...
            <Input type="number" min="0" value={variant.stock} onChange={(event) => updateVariant(index, "stock", Number(event.target.value))} placeholder="Quantity" />
        ...
        <p className="text-sm text-slate-600">Total quantity: {totalStock}</p>
      </div>

      <div className="grid gap-3 md:grid-cols-4">
        {[
          ["isPublished", "Show in shop"],
